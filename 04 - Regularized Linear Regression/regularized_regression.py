...
class RegularizedRegression(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = np.sum(np.square(y - prediction)) / (2 * N) + reg / 2 * (np.sum(np.square(W)))
    ...
    def poly_feature_transform(self,X,poly_order=1):
        ...
        """
        f_transform = np.append(np.ones((X.shape[0],1)), X, axis=1)
        
        for i in range(1, poly_order):
            f_transform = np.append(f_transform, np.power(X, i+1), axis=1)
